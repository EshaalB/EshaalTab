...
const DragDropEngine = (() => {
  ...
  const clearMarkers = () =>
    $$(".et-board-card.drop-before, .et-board-card.drop-after").forEach((el) =>
  ...
  function install(container) {
    ...
    container.addEventListener("dragleave", (e) => {
      e.target
        .closest(".et-board-card")
        ?.classList.remove("drop-before", "drop-after");
    });
    ...
    container.addEventListener("drop", (e) => {
      ...
      if (draggedType === "bookmark" && draggedBmInfo && board) {
        ...
        draggedType === "board" &&
        board &&
        board.dataset.id !== draggedBoardId
      ) {
        e.preventDefault();
...
const BoardRenderer = (() => {
  ...
  function boardLayoutMetrics() {
    ...
    const colW = Math.max(160, Math.min(preferredWidth, areaWidth - 24));
      ...
      if (header) {
        ...
        if (acc) {
          ...
          if (isExp) {
            ...
            data.collapsedBoards = data.collapsedBoards.filter(
              (id) => id !== boardId,
            );
          }
          StorageManager.save();
  ...
  function paintBoards() {
    {
      ...
      const collapsedBoards = Array.isArray(data.collapsedBoards)
      ...
      boards.forEach((board, index) => {
        ...
        colElements[colIdx].appendChild(
      ...
      colElements[boards.length % numCols].appendChild(addTile);
    ...
    setSafeHTML(
      ...
      <div class="et-board-card-header" role="button" tabindex="0"
        ...
        <div class="et-board-card-left">
          <button class="et-board-chevron" tabindex="-1" title="Toggle">
