...
const TabStash = (() => {
  ...
  async function stashCurrentWindow(closeAfter = true) {
    if (!(HAS_EXT && EXT.tabs)) {
      ToastSystem.error("Tab stashing needs the installed extension.");
      return null;
    }
    let tabs = [];
    try {
      tabs = (await EXT.tabs.query({ currentWindow: true })).filter(isWeb);
    } catch {}
    if (!tabs.length) {
      ToastSystem.info("No open web pages to stash.");
      return null;
    }
    ...
    tabs.forEach((t) =>
      BookmarkManager.add(board.id, t.title || t.url, t.url, ["session"]),
    );
    ...
    ToastSystem.success(
      `Stashed ${tabs.length} tab${tabs.length === 1 ? "" : "s"}`,
    );
    return board;
  }
